...
// idLen is a length of work id string.
// (20 bytes of 62-letter alphabet give ~119 bits.)
const idLen = 20

// idChars are characters allowed in work id.
...
func init() {
	if _, err := io.ReadFull(rand.Reader, rngKey[:]); err != nil {
		panic("work: error reading random source: " + err.Error())
	}
}
...
// pseudorandom numbers in range 0-9. The slice can be used as a work
// solution.
...
func randomBytesMod(length int, mod byte) (b []byte) {
	...
	if mod == 0 {
		panic("work: bad mod argument for randomBytesMod")
	}
